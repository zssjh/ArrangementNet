"""
Inference paper : UV_Net(https://arxiv.org/abs/2006.10211).
"""
import torch
...
class _EdgeConv(nn.Module):
    def __init__(
        ...
        self.proj = _MLP(1, hidden_mlp_dim, hidden_mlp_dim, hidden_mlp_dim)
        self.mlp = _MLP(num_mlp_layers, hidden_mlp_dim, hidden_mlp_dim, out_feats)
...
class UVNetGraphEncoder(nn.Module):
    def __init__(
        ...
        self.edge_node_conv_layers = torch.nn.ModuleList()
        ...
        for layer in range(self.num_layers - 1):
            ...
            self.edge_node_conv_layers.append(
                _NodeConv(
        ...
        self.linears_prediction = _MLP(2, hidden_dim, hidden_dim, output_dim)
        self.linears_prediction_e = _MLP(2, hidden_dim, hidden_dim, output_dim)
        self.econv = _MLP(1, input_edge_dim, hidden_dim, hidden_dim)
        self.drop1 = nn.Dropout(0.3)
        self.drop = nn.Dropout(0.5)
        self.pool = MaxPooling()

    def process(self, g, h, efeat, ge):
        hidden_rep = [h]
        hidden_rep_e = []
        he = efeat
        ...
        for i in range(self.num_layers - 1):
            ...
            identity = torch.ones(ge.edges()[0].shape[0], 1).float().cuda()
            he = self.edge_node_conv_layers[i](ge, he, identity)
            # Update edge features
            he = self.edge_conv_layers[i](g, h, he)
            hidden_rep.append(h)
            hidden_rep_e.append(he)

        out = hidden_rep[-1]
        out_e = hidden_rep_e[-1]
        out = self.drop1(out)

        out_e = self.drop1(hidden_rep_e[-1])
        out = self.linears_prediction(out)
        out_e = self.linears_prediction_e(out_e)
        return out, out_e

    def forward(self, g, ge):
        he = g.edata['x']
        h = g.ndata['x']
        return self.process(g, h, he, ge)
